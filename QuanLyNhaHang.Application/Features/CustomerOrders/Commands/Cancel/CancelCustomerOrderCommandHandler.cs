    ...
    : IRequestHandler<CancelCustomerOrderCommand, bool>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var isActiveCustomer = await _context.Users.AsNoTracking().AnyAsync(
            user => user.Id == customerUserId && user.Role == SystemRoles.Customer &&
        ...
        var order = await _context.Orders.FirstOrDefaultAsync(
            item => item.Id == request.OrderId && item.CustomerUserId == customerUserId && item.IsActive,
            cancellationToken);
        if (order == null) return false;
        if (order.Status == "Cancelled") return true;
        if (order.Status != "Pending")
            throw new InvalidOperationException("Bạn chỉ có thể tự hủy đơn khi đơn còn ở trạng thái Đang chờ. Nếu nhà hàng đã bắt đầu xử lý món, vui lòng liên hệ nhân viên để được hỗ trợ.");

        var hasPaidPayment = await _context.Payments.AsNoTracking().AnyAsync(
            payment => payment.OrderId == order.Id && payment.Status == "Paid",
            cancellationToken);
        if (hasPaidPayment)
            throw new InvalidOperationException("Đơn đã thanh toán nên không thể tự hủy trên website. Vui lòng liên hệ nhà hàng để được xử lý hoàn tiền/đối soát.");
        ...
        if (paymentAttempts.Any(attempt =>
            ...
            throw new InvalidOperationException("Đơn đang có giao dịch thanh toán cần đối soát nên chưa thể tự hủy. Vui lòng liên hệ nhà hàng để được hỗ trợ.");

        var orderItems = await _context.OrderItems
            .Where(item => item.OrderId == order.Id)
            .ToListAsync(cancellationToken);
        if (orderItems.Any(item => item.Status is "Cooking" or "Ready" or "Served"))
            throw new InvalidOperationException("Một hoặc nhiều món đã được bếp xử lý nên đơn không thể tự hủy.");
        ...
        {
            var hasOtherOpenOrderAtTable = await _context.Orders.AsNoTracking().AnyAsync(
                item => item.Id != order.Id && item.RestaurantTableId == order.RestaurantTableId &&
                        item.IsActive && item.Status != "Cancelled" && item.Status != "Completed",
                cancellationToken);
        ...
        {
            new(customerUserId, "Order.CancelledByCustomer", "Đã hủy đơn hàng",
                $"Đơn {order.OrderCode} đã được hủy theo yêu cầu của bạn.",
                "warning", "/orders", order.Id)
        };
        ...
        notifications.AddRange(adminUserIds.Select(userId => new Notification(
            userId, "Order.CancelledByCustomer", "Khách đã hủy đơn",
            $"Khách hàng vừa hủy đơn {order.OrderCode} khi đơn còn ở trạng thái chờ xử lý.",
            "warning", "Đơn hàng", order.Id)));
