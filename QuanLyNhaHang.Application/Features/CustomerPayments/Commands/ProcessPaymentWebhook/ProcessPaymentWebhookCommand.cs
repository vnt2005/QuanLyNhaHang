    ...
    : IRequestHandler<ProcessPaymentWebhookCommand, bool>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var transactionWasAfterExpiry = attempt.ExpiresAt < transaction.OccurredAtUtc;
            ...
            attempt.Status == PaymentAttempt.CreatingStatus)
        {
            ...
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, reason, "PAID");
        ...
        {
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, "AmountMismatch", "PAID");
            await _context.SaveChangesAsync(cancellationToken);
            return true;
        }

        var order = await _context.Orders.FirstOrDefaultAsync(item => item.Id == attempt.OrderId, cancellationToken);
        if (order == null)
        {
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, "OrderMissing", "PAID");
            await _context.SaveChangesAsync(cancellationToken);
            return true;
        }
        ...
        {
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, "PaidAfterOrderCancellation", "PAID");
            await _context.SaveChangesAsync(cancellationToken);
            return true;
        }

        var existingPayment = await _context.Payments.FirstOrDefaultAsync(
            payment => payment.OrderId == order.Id && payment.Status == "Paid",
            cancellationToken);
        if (existingPayment != null)
        {
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, "DuplicatePaymentAfterOrderAlreadyPaid", "PAID");
            await _context.SaveChangesAsync(cancellationToken);
            return true;
        }
        ...
        {
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, $"PaidWhenOrderStatusNotPayable:{order.Status}", "PAID");
            await _context.SaveChangesAsync(cancellationToken);
            return true;
        }
        ...
        {
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, $"QuoteUnavailable: {exception.Message}", "PAID");
            await _context.SaveChangesAsync(cancellationToken);
            return true;
        }
        ...
        {
            attempt.MarkRequiresReview(transaction.Amount, transaction.TransactionId, "OrderAmountChangedAfterPaymentRequestCreation", "PAID");
            await _context.SaveChangesAsync(cancellationToken);
            return true;
        }
        ...
        var payment = new Payment(
            ...
            quote.FinalAmount,
            "BankTransfer",
            $"{_paymentGateway.Provider} | transactionId={transaction.TransactionId} | reference={transaction.BankReference} | gateway={transaction.Gateway} | attempt={attempt.Id}",
            quote.ServiceChargeAmount);

        await _context.Payments.AddAsync(payment, cancellationToken);
        attempt.MarkPaid(payment.Id, transaction.Amount, transaction.TransactionId, "PAID");

        var promotionUsage = await _context.PromotionUsages.FirstOrDefaultAsync(
        ...
        if (order.OrderType == "Takeaway" && order.Status is "Ready" or "Served")
        {
            ...
            if (activeItems.Count > 0 && activeItems.All(item => item.Status is "Ready" or "Served"))
            {
        ...
        if (order.CustomerUserId.HasValue)
        {
            notifications.Add(new Notification(
                ...
                order.Status == "Completed"
                    ? $"Đơn {order.OrderCode} đã thanh toán {quote.FinalAmount.ToString("N0", CultureInfo.GetCultureInfo("vi-VN"))} đ qua {_paymentGateway.Provider} và đã hoàn tất."
                    : $"Đơn {order.OrderCode} đã thanh toán {quote.FinalAmount.ToString("N0", CultureInfo.GetCultureInfo("vi-VN"))} đ qua {_paymentGateway.Provider}.",
                "success",
                "/orders",
                order.Id));
        }
        ...
        notifications.AddRange(adminUserIds.Select(userId => new Notification(
            ...
            order.Status == "Completed"
                ? $"Đơn {order.OrderCode} vừa thanh toán {quote.FinalAmount.ToString("N0", CultureInfo.GetCultureInfo("vi-VN"))} đ qua {_paymentGateway.Provider} và đã tự động hoàn tất."
                : $"Đơn {order.OrderCode} vừa thanh toán {quote.FinalAmount.ToString("N0", CultureInfo.GetCultureInfo("vi-VN"))} đ qua {_paymentGateway.Provider}.",
            "success",
            "Hóa đơn",
            order.Id)));
