...
export default function ActivityLogsPage({ role }: { role?: string }) {
  ...
  return (
    <section className="activity-logs-page">
      ...
      {error && <div className="activity-alert error">{error}<button type="button" aria-label="Đóng thông báo lỗi" onClick={() => setError('')}>×</button></div>}
      {message && <div className="activity-alert success">{message}<button type="button" aria-label="Đóng thông báo thành công" onClick={() => setMessage('')}>×</button></div>}
